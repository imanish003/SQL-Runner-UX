...
- **Future-Proofing**: Both libraries are actively maintained and have strong community support, ensuring they will continue to receive updates and improvements. We can easily add features like sorting, filtering, pagination etc. in future. It will be very lower effort to add these features as compared to building a custom table from scratch.

### **State Management**: React Hooks like `useState`

I decided to not use any state management libraries like Redux or Zustand for now. The application state will be managed locally using React hooks like `useState`, which are provided by React itself. This will keep things simple and avoid unnecessary complexity. If needed, we can easily add global state management libraries in future.

### **Styling**: Tailwind CSS v4

Tailwind CSS is a utility-first CSS framework that allows for rapid UI development. It provides a set of pre-defined classes that can be composed to create custom designs without writing custom CSS. We should use ShadCN components for most of the UI, but Tailwind CSS can be used for custom styling as needed.
